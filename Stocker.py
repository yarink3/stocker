...
def send_confirmation_code(randomForConfirmaionMail):
	"""A function used for sending confirmation mail to the user un order to verify her/his email address.
	...
	if('@' not in userMail):
		...
		try:
			...
			create_pop_window("Error","Something went wrong.\nMaybe you typed wrong mail or don't have internet connection",'530x200','ok',"","")			
...
def delete_selected_stocks(listbox):
...
def set_stocks_list(listbox):
	""" A function used to set the users stocks list when opening the software.
	...
	follow_list=followSheet.findall(userMail)
	for followed in follow_list:
		listbox.insert(END,followSheet.cell(followed.row,followed.col+1).value + " --> "+followSheet.cell(followed.row,followed.col+2).value)
...
def main_window():
	...
	Label(user_details_frame,text='User Name :',relief="solid",font=labelFontBold,background=screenBackground).grid(column=0,row=0) 
	Label(user_details_frame, text='Email :',relief="solid",font=labelFontBold,background=screenBackground).grid(column=0,row=1) 

	global userName
	global userMail
	userNameLabel = Label(user_details_frame, text=userName,font=labelFontBold, relief="solid",background=screenBackground).grid( column=1,row=0,padx=70)
	userMailLabel= Label(user_details_frame, text=userMail,font=labelFontBold, relief="solid",background=screenBackground).grid( column=1,row=1,padx=70)
	log_out_button=Button(user_details_frame,font=labelFont, text='log out',background=buttonGrayBackground,command=changeMail).grid( column=2 ,row=0)
	sign_out_button=Button(user_details_frame,font=labelFont, text='sign out',background=buttonGrayBackground,command=changeMail).grid( column=2,row=1)
	...
	global stockNameEntry
	# stockNameEntry=Entry(search_frame)
	...
	# stockNameEntry.font=labelFont
	stockNameEntry.grid(column=1 ,row=0)

	# LabelTab= Label(master, text=" LINE LINE LINE LINE LINE",fg=screenBackground, background=screenBackground).grid(row=6, column=0) 
	search_button=Button(search_frame, text="search",font=labelFont ,background="#00ff00",command=lambda: searchClicked(stocks_list)).grid( column=2,row=0,padx=30)
	# stop_button=Button(search_frame, text="stop",font=labelFont,background="#ff0000",command=stopClicked).grid(column=3,row=0) # TODO - delete and add list

	stocks_list_frame=Frame(master,bg=screenBackground)
	stocks_list_frame.grid(column=0,row=4)
	# stocks_list_frame['bg'] = screenBackground

	stocks_list=Listbox(stocks_list_frame ,bg='#FFFFFF',selectmode="multiple" ,bd=2,font=labelFont)
	set_stocks_list(stocks_list)
	...
	delete_button=Button(stocks_list_frame, text="Stop follow and delete selected",font=labelFont,background="#ff0000",command=lambda: delete_selected_stocks(stocks_list))
	# stop_button=Button(stocks_list_frame, text="stop follow selected",font=labelFont,background="#ff0000",command=lambda: delete_selected_stocks(stocks_list))

	delete_button.pack(side=BOTTOM)

	list_title=Label(stocks_list_frame,text="My stocks list( stock name --> my price ) ",borderwidth=1, relief="ridge",font=labelFontBold)
	...
	mainThread=threading.Thread(target=new_function,daemon=True)
	mainThread.start()
...
def login():
	...
	if nameOk==1 and str(password_to_check) != str(usersSheet.cell(usersSheet.find(user_name_to_check).row,usersSheet.find(user_name_to_check).col+2 ).value):
		can_login=0
		errorMsg=errorMsg+"Something Went wrong, check username and password - 2 "
	if(can_login==0):
		create_pop_window("Login Error",errorMsg,'360x120',"Try again","","")
			...
			userMail=usersSheet.cell(usersSheet.find(user_name_to_check).row,usersSheet.find(user_name_to_check).col+1 ).value
			print(userMail)
			main_window()


def getStockName(alsoGetPrice):
	global stockName
	global currentPrice
	global stockNameEntry

	stockNameSearch=str(stockNameEntry.get())


	url = "https://finance.yahoo.com/lookup?s={}".format(stockNameSearch)
	source = requests.get(url).text
	soup = BeautifulSoup(source, 'lxml')
	stockName = soup.find('td', attrs={"data-reactid": "58"}) # 57+8*n
	if(stockNameSearch=="" or stockName is None):
		return ""
	else:	
...
sendCodeButton= Button(logRegWindow, text='Send code',font=("David",11),width=7,height=1,command=lambda: send_confirmation_code(randomForConfirmaionMail))


userNameEntry.grid(row=0,column=1)
passwordEntry.grid(row=2,column=1)

registerButton=Button(logRegWindow, text='Registration',command=RegistrationSelected,background=buttonGrayBackground)
registerButton.grid(row=5,column=1)

loginButton=Button(logRegWindow, text='Login',command=login,background=buttonGrayBackground)
...
def add_to_stocks_list(listbox,stock_name,line1,limit_asker,currentPrice):
...
def searchClicked(listbox):
	stock_name_and_price=getStockName(1)
	...
	if(stockName==""):
		...
		line1= Label(stock_deatail_frame_name_and_price,font=labelFont, text=stock_name_and_price,background=screenBackground)
		...
		line2= Label(stock_deatail_frame_limit_asker,font=labelFont, text='Let me now when it is under / over: (e.g. 12.34) ',background=screenBackground)
		line2.grid(row=0,column=0)

		limitAsker=Entry(stock_deatail_frame_limit_asker)
		limitAsker.grid(row=0,column=1)	

		b3=Button(stock_deatail_frame_limit_asker,font=labelFont, text='Add to my follow list', height=0,background="#00ff00",command=lambda: add_to_stocks_list(listbox,stockName,line1,limitAsker,float(currentPrice)))
